...
- **再生** — 再生/停止、15秒スキップ、30秒巻き戻し、シークバー、再生位置の自動保存・復元。
  ...
  左上の「∨」で再生を続けたまま閉じて他の画面を操作できる。
  ページを閉じて開き直しても最後のエピソードがプレイヤーに復元され、中断位置から再開できる。
  プレイヤー左下の設定ボタンから再生設定シートを開き、番組ごとの再生速度
  （1.0x / 1.1x / 1.2x・端末間同期対応）とスリープタイマー（20分で自動停止）を設定できる。
  設定ボタン横に現在の倍速・タイマー残り時間を要約表示
- **タイムスタンプジャンプ** — エピソード概要内の `3:12` `1:02:33` 形式の時刻表記を
  自動でリンク化し、タップでその位置から再生
